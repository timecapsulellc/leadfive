...
class LeadFiveFeatureValidator {
  ...
  async testPrivateKeySecurity() {
    ...
    function scanDirectory(dir) {
      ...
      for (const item of items) {
        ...
        } else if (stat.isFile() && (item.endsWith('.js') || item.endsWith('.jsx') || item.endsWith('.ts'))) {
          try {
    ...
    return {
      ...
      message: hasPrivateKeys ? 'SECURITY RISK: Private keys found in source code' : 'No private keys found in source code'
    };
  }
  ...
  async testRateLimiting() {
    ...
    const packageJson = fs.readFileSync('package.json', 'utf8');
    const hasRateLimit = packageJson.includes('express-rate-limit') || 
                        packageJson.includes('rate-limit');

    return {
      passed: hasRateLimit,
      message: hasRateLimit ? 'Rate limiting middleware installed' : 'Rate limiting not implemented'
    };
  }
  ...
  async testCSRFProtection() {
    ...
    const packageJson = fs.readFileSync('package.json', 'utf8');
    const hasCSRF = packageJson.includes('csrf') || packageJson.includes('csurf');

    return {
      passed: hasCSRF,
      message: hasCSRF ? 'CSRF protection installed' : 'CSRF protection not implemented'
    };
  }
