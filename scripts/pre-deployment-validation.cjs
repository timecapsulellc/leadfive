...
/**
 * 🛡️ ORPHI CROWDFUND MAINNET PRE-DEPLOYMENT VALIDATION SCRIPT
 * 
 * This script performs comprehensive validation before mainnet deployment:
...
async function validateEnvironment() {
    ...
    if (!fs.existsSync('.env')) {
        printError("No .env file found! Create one from .env.mainnet template");
        envValid = false;
    } else {
...
async function validateAccountBalance() {
    printSection("Account Balance & Security Validation");
    ...
    try {
        ...
        // Check if address matches expected Trezor admin
        if (deployerAddress.toLowerCase() === MAINNET_CONFIG.TREZOR_ADMIN.toLowerCase()) {
            printSuccess("Deployer is the Trezor admin wallet");
        } else {
            printWarning("Deployer is not the Trezor admin wallet");
            console.log(`   Expected: ${MAINNET_CONFIG.TREZOR_ADMIN}`);
...
async function validateContracts() {
    ...
    try {
        ...
        // Validate Trezor admin address
        console.log("\n🔐 Trezor Admin Validation:");
        console.log(`   Address: ${MAINNET_CONFIG.TREZOR_ADMIN}`);
        
        // Check if it's a valid address
        if (ethers.utils.isAddress(MAINNET_CONFIG.TREZOR_ADMIN)) {
            printSuccess("Trezor admin address is valid");
        } else {
            printError("Trezor admin address is invalid");
            validationResults.contracts = false;
            return false;
        }
...
async function validateSecurity() {
    ...
    if (MAINNET_CONFIG.TREZOR_ADMIN && ethers.utils.isAddress(MAINNET_CONFIG.TREZOR_ADMIN)) {
        printSuccess("Trezor admin wallet properly configured");
        securityScore += 1;
    } else {
        printError("Invalid Trezor admin wallet configuration");
    }
    ...
    console.log("\n💾 Backup Strategy:");
    if (process.env.BACKUP_DEPLOYER_PRIVATE_KEY) {
        printSuccess("Backup deployer configured");
        securityScore += 1;
    } else {
        printWarning("No backup deployer configured");
        securityScore += 0.5;
    }
...
async function runPreDeploymentValidation() {
    console.log("🛡️ ORPHI CROWDFUND MAINNET PRE-DEPLOYMENT VALIDATION");
    console.log("═".repeat(80));
    console.log("🎯 Ensuring 100% readiness for BSC Mainnet deployment");
    ...
    try {
        ...
        if (validationResults.overall) {
            ...
            console.log("\n📋 Next Steps:");
            console.log("1. Run: npx hardhat run scripts/deploy-mainnet-comprehensive.js --network bsc");
        ...
        const reportPath = `pre-deployment-validation-${Date.now()}.json`;
...
module.exports = { runPreDeploymentValidation, validationResults };
