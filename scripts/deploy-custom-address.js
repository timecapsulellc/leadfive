...
const DEPLOYMENT_CONFIG = {
    ...
    MIN_BNB_BALANCE: "0.1",
    TARGET_ADDRESS: "0x7FB9622c6b2480Fd75b611b87b16c556A10baA01"
};
...
const saveDeploymentInfo = (deploymentData) => {
    ...
    const filename = `orphi-custom-mainnet-${timestamp}.json`;
...
async function main() {
    console.log("\n" + "=".repeat(80));
    console.log("🚀 ORPHI CROWDFUND BSC MAINNET DEPLOYMENT - CUSTOM ADDRESS");
    ...
    try {
        ...
        console.log(`👤 Custom Deployer: ${deployer.address}`);
        
        // Verify the address matches expected
        if (deployer.address.toLowerCase() !== DEPLOYMENT_CONFIG.TARGET_ADDRESS.toLowerCase()) {
            console.log(`⚠️  Address Mismatch!`);
            console.log(`   Expected: ${DEPLOYMENT_CONFIG.TARGET_ADDRESS}`);
            console.log(`   Got: ${deployer.address}`);
            console.log(`   Please check your private key in .env.custom file`);
        } else {
            console.log(`✅ Address Verified: ${DEPLOYMENT_CONFIG.TARGET_ADDRESS}`);
        }
        ...
        console.log(`   • Owner: ${deployer.address} ✅ CUSTOM`);
        console.log(`   • Treasury: ${deployer.address} ✅ CUSTOM`);
        console.log(`   • Emergency: ${deployer.address} ✅ CUSTOM`);
        console.log(`   • Pool Manager: ${deployer.address} ✅ CUSTOM`);
        console.log(`   • All admin roles assigned to your custom wallet`);
        
        // Get contract factory
        console.log("\n📦 Loading contract factory...");
        const ContractFactory = await ethers.getContractFactory(DEPLOYMENT_CONFIG.CONTRACT_NAME);
        
        // Prepare initialization arguments - ALL ROLES = CUSTOM ADDRESS
        const initArgs = [
            DEPLOYMENT_CONFIG.USDT_ADDRESS,  // _usdtToken
            deployer.address,                // _treasuryAddress (Custom)
            deployer.address,                // _emergencyAddress (Custom)
            deployer.address                 // _poolManagerAddress (Custom)
        ];
        
        console.log("\n📋 Initialization Arguments:");
        console.log(`   • USDT Token: ${initArgs[0]}`);
        console.log(`   • Treasury: ${initArgs[1]} ✅ CUSTOM`);
        console.log(`   • Emergency: ${initArgs[2]} ✅ CUSTOM`);
        console.log(`   • Pool Manager: ${initArgs[3]} ✅ CUSTOM`);
        ...
        const deploymentData = {
            ...
            targetAddress: DEPLOYMENT_CONFIG.TARGET_ADDRESS,
        ...
        console.log(`   npx hardhat verify --network bsc ${contract.address} "${DEPLOYMENT_CONFIG.USDT_ADDRESS}" "${deployer.address}" "${deployer.address}" "${deployer.address}"`);
        ...
        console.log("• All admin roles secured with your custom address");
        console.log("• Keep your private key safe and backed up");
        ...
        if (error.message.includes("insufficient funds")) {
            ...
            console.error(`💡 Send BNB to: ${DEPLOYMENT_CONFIG.TARGET_ADDRESS}`);
        } else if (error.message.includes("user rejected")) {
            console.error("\n💡 Solution: Confirm the transaction in your wallet");
        } else if (error.message.includes("network")) {
            console.error("\n💡 Solution: Check your BSC Mainnet connection");
        } else if (error.message.includes("private key")) {
            console.error("\n💡 Solution: Check your private key in .env.custom file");
        }
