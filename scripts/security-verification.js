...
function checkEnvProtection() {
    ...
    try {
        ...
        if (fs.existsSync('.env')) {
            ...
            if (envContent.includes('DEPLOYER_PRIVATE_KEY=') && !envContent.includes('your_actual_64_character')) {
                ...
                if (privateKeyValue && privateKeyValue.length > 10) {
...
function checkSourceCodeSecurity() {
    ...
    const searchPatterns = [
        { pattern: /[0-9a-fA-F]{64}/, name: "64-character hex strings (potential private keys)" },
        { pattern: /0x[0-9a-fA-F]{40}/, name: "Ethereum addresses" },
        { pattern: /sk-[a-zA-Z0-9]{48}/, name: "OpenAI API keys" },
    ...
    filesToCheck.forEach(fileOrDir => {
        if (fs.existsSync(fileOrDir)) {
            searchPatterns.forEach(({ pattern, name }) => {
                try {
                    const result = execSync(`grep -r "${pattern.source}" ${fileOrDir} --exclude-dir=node_modules --exclude="*.md" || true`, { encoding: 'utf8' });
                    if (result.trim()) {
                        ...
                        if (lines.length > 0) {
                            securityIssues.push(`⚠️  Potential ${name} found in ${fileOrDir}`);
...
function checkDeploymentSecurity() {
    ...
    try {
        if (fs.existsSync('scripts/deploy-leadfive.js')) {
            ...
            if (deployScript.includes('process.env.DEPLOYER_PRIVATE_KEY')) {
            ...
            // Check for require('dotenv')
            if (deployScript.includes('require') && deployScript.includes('dotenv')) {
                securityPassed.push("✅ Deployment script loads environment safely");
            }
