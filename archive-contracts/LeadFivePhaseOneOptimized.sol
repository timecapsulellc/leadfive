    ...
    PausableUpgradeable 
{
    ...
    function _initializePackages() private {
        ...
        packages[1] = CoreOptimized.PackedPackage({
            ...
            directBonus: 4000, // 40%
            levelBonus: 1000, // 10%
            uplineBonus: 0, // Not used in this business plan
            leaderBonus: 1000, // 10%
            helpBonus: 1000, // 10%
            clubBonus: 1000 // 10%
        });
    ...
    function _processPayment(uint8 packageLevel, bool useUSDT) internal returns (uint96) {
        ...
        if(useUSDT) {
            require(usdt.transferFrom(msg.sender, address(this), packagePrice), "TX fail");
            return packagePrice;
        } else {
    ...
    function setPlatformFeeRecipient(address _recipient) external onlyOwner {
        require(_recipient != address(0), "0");
