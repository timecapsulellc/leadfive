...
const saveDeploymentInfo = (deploymentData) => {
    ...
    const filename = `orphi-mainnet-${timestamp}.json`;
...
async function main() {
    console.log("\n" + "=".repeat(80));
    console.log("🚀 ORPHI CROWDFUND BSC MAINNET DEPLOYMENT");
    ...
    try {
        ...
        if (!signers || signers.length === 0) {
            throw new Error("❌ No signers available! Please check your private key in .env.custom file");
        }
        
        const deployer = signers[0];
        if (!deployer || !deployer.address) {
            throw new Error("❌ Deployer wallet not found! Please check your DEPLOYER_PRIVATE_KEY in .env.custom file");
        }
        ...
        const gasPrice = ethers.utils ? ethers.utils.parseUnits(DEPLOYMENT_CONFIG.GAS_PRICE, "wei") : ethers.parseUnits(DEPLOYMENT_CONFIG.GAS_PRICE, "wei");
        const estimatedCost = gasPrice * BigInt(DEPLOYMENT_CONFIG.GAS_LIMIT);
        ...
        const actualCost = BigInt(receipt.gasUsed) * BigInt(receipt.gasPrice || receipt.effectiveGasPrice || gasPrice);
        
        console.log("\n📊 Deployment Results:");
        console.log(`   • Gas Used: ${actualGasUsed.toLocaleString()}`);
        console.log(`   • Gas Price: ${formatGwei(receipt.gasPrice || receipt.effectiveGasPrice || gasPrice)} gwei`);
        ...
        if (error.message.includes("insufficient funds")) {
            ...
            console.error("\n💡 Solution: Confirm the transaction on your wallet");
