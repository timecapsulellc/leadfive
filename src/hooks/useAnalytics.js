...
export function useAnalytics(teamData, compensationData) {
  ...
  const applyFilters = React.useCallback((data, filters) => {
    ...
    Object.keys(data.levels).forEach(level => {
      ...
      if (
        levelNum >= filters.levelRange.min &&
        levelNum <= filters.levelRange.max
      ) {
    ...
    // Recalculate totals
    filteredData.totalMembers = Object.values(filteredLevels).reduce(
      (sum, level) => sum + level.count,
      0
    );
    filteredData.activeMembers = Object.values(filteredLevels).reduce(
      (sum, level) => sum + level.activeCount,
      0
    );
    filteredData.totalVolume = Object.values(filteredLevels).reduce(
  ...
  const exportTeamData = React.useCallback(
    (format = 'json') => {
      const exportData = {
        ...
        teamStructure: Object.keys(teamData.levels).map(level => ({
          ...
          performance: Math.round(
            (teamData.levels[level].activeCount /
              teamData.levels[level].count) *
              100
          ),
        })),
        compensation: {
