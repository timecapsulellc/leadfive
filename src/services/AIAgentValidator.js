...
class AIAgentValidator {
    ...
    async validateContractQuery(queryData, userContext) {
        ...
        if (this.validationRules.contract.requiresAuth.includes(method) && !userAddress) {
        ...
        try {
            const gasEstimate = this.validationRules.contract.gasLimits[method] || 100000;
            ...
            if (this.validationRules.contract.requiresAuth.includes(method)) {
                ...
                const result = await this.compensationService.contract[method](...params);
                return {
                    method: method,
                    params: params,
    ...
    async validateCompensationQuery(queryData, userContext) {
        const { calculationType, userAddress, amount, level, packageLevel } = queryData;

        switch (calculationType) {
            case 'direct_commission':
                return await this.validateDirectCommission(userAddress, amount, packageLevel);
            ...
            default:
                throw new Error(`Unknown compensation calculation type: ${calculationType}`);
        }
    }
    ...
    async validateDirectCommission(userAddress, amount, packageLevel) {
        ...
        if (packageLevel < 1 || packageLevel > this.validationRules.compensation.maxPackageLevel) {
            throw new Error(`Package level must be between 1 and ${this.validationRules.compensation.maxPackageLevel}`);
        }

        const compensation = await this.compensationService.validateCompensation(userAddress, amount, 1);
    ...
    async validateLevelCommission(userAddress, amount, level) {
        if (level < 1 || level > this.validationRules.business.maxReferralDepth) {
            throw new Error(`Level must be between 1 and ${this.validationRules.business.maxReferralDepth}`);
        }
    ...
    async validateMatrixSpillover(queryData) {
        ...
        if (!this.isValidAddress(sponsorAddress) || !this.isValidAddress(newMemberAddress)) {
    ...
    async validateEarningsCap(userAddress, packageLevel) {
        const userInfo = await this.compensationService.contract.getUserInfo(userAddress);
    ...
    async validateRankAdvancement(data) {
        ...
        for (const [rank, requirements] of Object.entries(criteria)) {
            if (currentVolume >= requirements.volume && currentReferrals >= requirements.referrals) {
    ...
    async validatePoolDistribution(data) {
        ...
        return {
            ...
            pools: poolBalances,
            distributionFrequency: this.validationRules.business.poolDistributionFrequency,
            validated: true
        };
    }
    ...
    async validateReferralTree(data) {
        ...
        const treeData = await this.compensationService.getReferralTreeData(rootAddress, depth);
    ...
    async validateRegistrationAction(data) {
        ...
        const packageValidation = await this.compensationService.validatePackageRegistration(
            packageLevel, 
            paymentMethod
        );
    ...
    async validateWithdrawalAction(userAddress, data) {
        ...
        if (amount < this.validationRules.compensation.minWithdrawalAmount) {
            throw new Error(`Minimum withdrawal amount is ${this.validationRules.compensation.minWithdrawalAmount} USDT`);
        }

        if (amount > this.validationRules.compensation.maxWithdrawalPerDay) {
            throw new Error(`Maximum daily withdrawal is ${this.validationRules.compensation.maxWithdrawalPerDay} USDT`);
        }
    ...
    async validateClaimAction(userAddress) {
        const pendingRewards = await this.compensationService.contract.getPendingRewards(userAddress);
    ...
    updateMetrics(success, responseTime) {
        ...
        const totalTime = this.accuracyMetrics.averageResponseTime * (this.accuracyMetrics.totalQueries - 1) + responseTime;
        this.accuracyMetrics.averageResponseTime = totalTime / this.accuracyMetrics.totalQueries;
    }
     ...
     */
    getAccuracyMetrics() {
        const accuracy = this.accuracyMetrics.totalQueries > 0 
            ? (this.accuracyMetrics.validQueries / this.accuracyMetrics.totalQueries) * 100 
            : 0;
