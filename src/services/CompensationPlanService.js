...
export class CompensationPlanService {
  ...
  static WITHDRAWAL_TIERS = {
    0: { withdrawal: 0.70, reinvestment: 0.30 },   // No direct referrals
    5: { withdrawal: 0.75, reinvestment: 0.25 },   // 5 direct referrals
    20: { withdrawal: 0.80, reinvestment: 0.20 }   // 20 direct referrals
  };

  // Reinvestment allocation
  static REINVESTMENT_ALLOCATION = {
    levelBonus: 0.40,      // 40%
    globalUpline: 0.30,    // 30%
    globalHelpPool: 0.30   // 30%
  };
  ...
  static calculateCompensationBreakdown(packageAmount) {
    return {
      sponsorCommission: packageAmount * 0.40,    // 40%
      levelBonus: packageAmount * 0.10,           // 10%
      globalUplineBonus: packageAmount * 0.10,    // 10%
      leaderBonus: packageAmount * 0.10,          // 10%
      globalHelpPool: packageAmount * 0.30        // 30%
    };
  }
  ...
  static calculateLevelBonusDistribution(packageAmount) {
    const totalLevelBonus = packageAmount * 0.10;
    const distribution = {};
    
    Object.entries(this.LEVEL_BONUS_STRUCTURE).forEach(([level, percentage]) => {
      distribution[level] = totalLevelBonus * (percentage / 0.10); // Normalize to 10%
    });
  ...
  static calculateGlobalUplineBonus(packageAmount) {
    const totalUplineBonus = packageAmount * 0.10;
  ...
  static checkLeaderQualification(teamSize, directReferrals) {
    ...
    if (teamSize >= this.LEADER_REQUIREMENTS.SHINING_STAR.teamMembers && 
   ...
   */
  static calculatePotentialEarnings(packageAmount, teamSize, directReferrals = 0) {
    ...
    return {
      ...
      earningsCap,
      estimatedMonthly: {
        sponsorCommissions: Math.min(estimatedMonthlyFromTeam * 0.4, earningsCap * 0.3),
        levelBonuses: Math.min(estimatedMonthlyFromTeam * 0.1, earningsCap * 0.2),
        globalUpline: breakdown.globalUplineBonus,
        globalHelpPool: estimatedGlobalHelpPool,
        total: function() {
          return this.sponsorCommissions + this.levelBonuses + this.globalUpline + this.globalHelpPool;
        }
      }
    };
  }
  ...
  static generateGenealogyTreeData(userPackage = 100, depth = 5) {
    const generateNode = (level, position, parentPackage) => {
      ...
      if (level < depth) {
        ...
        const rightChild = generateNode(level + 1, position * 2 + 1, randomPackage);
   ...
   */
  static calculateDashboardData(userPackage = 100, teamSize = 47, directReferrals = 5) {
    ...
    const leaderStatus = this.checkLeaderQualification(teamSize, directReferrals);
    ...
    return {
      ...
      rank: {
        current: leaderStatus.length > 0 ? leaderStatus[0].replace('_', ' ') : 'Member',
