...
class ElevenLabsService {
  constructor() {
    ...
    this.isConversationalAIEnabled = !!this.apiKey && this.apiKey !== 'your_elevenlabs_api_key_here';
    ...
    this.personalityVoices = {
      'advisor': {
        ...
        personality: 'Professional, confident financial advisor with warm authority'
      },
      'analyzer': {
  ...
  async generateConversationalResponse(text, personality = 'advisor', userContext = {}) {
    if (!this.isConversationalAIReady()) {
      console.log('ElevenLabs Conversational AI not available, using fallback');
  ...
  async airaPersonalityResponse(text, personality = 'advisor', userContext = {}) {
    console.log(`[AIRA Voice] Generating ${personality} response for: "${text.substring(0, 50)}..."`);
    ...
    return this.generateConversationalResponse(voiceText, personality, userContext);
  }
  ...
  async testPersonalityVoice(personality = 'advisor') {
    const testMessages = {
      advisor: "Hello! I'm your Revenue Advisor. Let me help you optimize your earnings and build financial freedom through LeadFive's proven 4X system.",
      analyzer: "Greetings! As your Network Analyzer, I provide data-driven insights to maximize your team performance and growth metrics.",
      mentor: "Hey there! I'm your Success Mentor, here to inspire and guide you on your journey to financial independence. You've got this!",
      strategist: "Welcome! I'm your Binary Strategist, focused on architecting your long-term wealth building plan with systematic precision."
    };
    ...
    try {
      ...
      switch(voiceConfig.voiceId) {
        ...
        case 'friendly_assistant':
          selectedVoice = voices.find(v => 
            v.lang.includes('en') && 
            (v.name.includes('Karen') || v.name.includes('Siri') || v.name.includes('Assistant'))
          );
          break;
        default:
  ...
  getServiceStatus() {
    return {
      ...
      hasElevenLabsKey: !!this.apiKey && this.apiKey !== 'your_elevenlabs_api_key_here',
