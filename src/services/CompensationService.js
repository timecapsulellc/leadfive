...
class CompensationService {
    ...
    async init() {
        try {
            this.provider = new ethers.JsonRpcProvider(APP_CONFIG.contract.network.rpcUrl);
    ...
    async validateCompensation(userAddress, transactionAmount, level = 1) {
        try {
            ...
            const packageInfo = await this.contract.getPackageInfo(userInfo.packageLevel);
            ...
            return {
                ...
                remainingCap: Number(userInfo.earningsCap) - Number(userInfo.totalEarnings),
                timestamp: new Date().toISOString(),
                isValid: userInfo.isRegistered && !userInfo.isBlacklisted && userInfo.isActive
            };
        } catch (error) {
     ...
     */
    async handleMatrixSpillover(sponsorAddress, newMemberAddress, matrixPosition) {
        try {
            ...
            if (spilloverLogic.spilloverRequired && spilloverLogic.uplineAddress !== ethers.ZeroAddress) {
                // Find next available position in upline
                const uplineInfo = await this.contract.getUserInfo(spilloverLogic.uplineAddress);
