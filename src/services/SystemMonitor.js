...
class SystemMonitor {
  ...
  startMonitoring(intervalMs = 120000) {
    // 2 minutes default (reduced from 30 seconds)
   ...
   */
  getMetricStats(metricName, timeframeMs = 3600000) {
    // 1 hour default
    ...
    for (const metricName of keyMetrics) {
      ...
      if (stats) {
        ...
        if (threshold && stats.average > threshold) {
          health.issues.push(
            `${metricName} average (${stats.average}) exceeds threshold (${threshold})`
          );
   ...
   * Get recent alerts
   */
  getRecentAlerts(timeframeMs = 3600000) {
    // 1 hour default
    const cutoffTime = new Date(Date.now() - timeframeMs).toISOString();
   ...
   */
  exportData(timeframeMs = 86400000) {
    // 24 hours default
    const cutoffTime = new Date(Date.now() - timeframeMs).toISOString();
  ...
  clearOldData(ageMs = 604800000) {
    ...
    for (const [metricName, metricData] of Object.entries(this.metrics)) {
      ...
      this.metrics[metricName] = metricData.filter(
        entry => entry.timestamp >= cutoffTime
      );
