  ...
  const updateStatus = () => {
    setStatus({
      ...
      elevenlabs: {
        ...
        message: ElevenLabsService.isReady()
          ? 'Connected and ready'
      ...
      if (audio.success) {
        ...
        setTestResults(prev => ({
          ...
          elevenlabs: {
            success: true,
            message:
              'Voice synthesis test successful! Audio should be playing.',
    ...
    <div className="ai-settings max-w-4xl mx-auto p-6 space-y-8">
        ...
        <div className="flex justify-center items-center mb-4">
          ...
          <div>
            ...
            <p className="text-silver-mist">
              Configure ChatGPT and ElevenLabs integration
            </p>
          </div>
        </div>
        <div className="text-sm text-silver-mist/70">
          Developed by{' '}
          <span className="text-cyber-blue font-semibold">LEAD 5</span> - Young
      ...
      <div className="status-grid grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="status-card glass-morphism rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div
              className={`status-indicator ${status.openai.connected ? 'connected' : 'disconnected'}`}
            >
              <i
                className={`fas ${status.openai.connected ? 'fa-check-circle' : 'fa-times-circle'}`}
              ></i>
            </div>
          </div>
          <p className="text-silver-mist text-sm mb-2">
        ...
        <div className="status-card glass-morphism rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div
              className={`status-indicator ${status.elevenlabs.connected ? 'connected' : 'fallback'}`}
            >
              <i
                className={`fas ${status.elevenlabs.connected ? 'fa-check-circle' : 'fa-exclamation-triangle'}`}
              ></i>
            </div>
          </div>
          <p className="text-silver-mist text-sm mb-2">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              >
                <i
                  className={`fas ${showOpenaiKey ? 'fa-eye-slash' : 'fa-eye'}`}
                ></i>
              </button>
            </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              >
                <i
                  className={`fas ${showElevenLabsKey ? 'fa-eye-slash' : 'fa-eye'}`}
                ></i>
              </button>
            </div>
            <p className="text-silver-mist/70 text-sm mt-2">
      ...
      {testResults && (
        <div className="test-results glass-morphism rounded-2xl p-6">
          ...
          {testResults.openai && (
            <div
              className={`test-result mb-4 p-4 rounded-lg ${testResults.openai.success ? 'bg-success-green/20 border border-success-green/50' : 'bg-alert-red/20 border border-alert-red/50'}`}
            >
          ...
          {testResults.elevenlabs && (
            <div
              className={`test-result p-4 rounded-lg ${testResults.elevenlabs.success ? 'bg-success-green/20 border border-success-green/50' : 'bg-alert-red/20 border border-alert-red/50'}`}
            >
              <div className="flex items-center justify-between mb-2">
                <h4 className="font-semibold text-white">ElevenLabs Test</h4>
                <span className="text-sm text-silver-mist">
                  {testResults.elevenlabs.timestamp}
                </span>
              </div>
              {testResults.elevenlabs.success ? (
                <div>
                  ...
                  <p className="text-silver-mist text-xs mt-1">
                    Service: {testResults.elevenlabs.service}
                  </p>
                </div>
              ) : (
                <p className="text-alert-red text-sm">
                  Error: {testResults.elevenlabs.error}
                </p>
              )}
            </div>
          )}
