...
const AIEnhancedDashboard = ({ account, contractData, isConnected }) => {
  ...
  const [voiceGreeting, setVoiceGreeting] = useState(
    `Welcome to LeadFive, ${account ? account.slice(0, 6) : 'friend'}! AI + Web3 are revolutionizing networking—and you're at the forefront. Let's turn your ideas into reality!`
  );
  ...
  const askAI = async () => {
    ...
    try {
      const response = await OpenAIService.getChatResponse(
        "Give me a motivational insight about my Web3 networking journey!",
        userContext
      );
