...
const RealtimeStatus = ({ onUpdate }) => {
  ...
  useEffect(() => {
    ...
    const handleEarningsUpdate = data => {
      ...
      setUpdateCount(prev => prev + 1);
      addNotification(
        `Earnings updated: $${data.earnings?.totalEarned?.toFixed(2) || '0.00'}`,
        'success'
      );
    ...
    const handleNewReferral = data => {
      ...
      setUpdateCount(prev => prev + 1);
      addNotification(
        `New referral from ${data.sponsor?.substring(0, 8)}...`,
        'success'
      );
    ...
    return () => {
      ...
      wsService.removeListener(
        wsService.events.EARNINGS_UPDATED,
        handleEarningsUpdate
      );
      wsService.removeListener(
        wsService.events.NEW_REFERRAL,
        handleNewReferral
      );
      wsService.removeListener(
        wsService.events.NETWORK_STATS,
        handleNetworkStats
      );
    };
  }, [onUpdate]);
