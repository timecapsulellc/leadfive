...
const searchTree = (node, query) => {
  ...
  const searchNode = (current, path = []) => {
    ...
    const nodeText = [
      ...
      current.attributes?.packageTier
        ? PACKAGE_TIER_LABELS[current.attributes.packageTier]
        : '',
    ]
      .join(' ')
      .toLowerCase();
...
const NetworkTreeVisualization = ({
  ...
  const [currentZoom, setCurrentZoom] = useState(
    DEFAULT_CONFIG.initialZoom || 0.8
  );
  ...
  const renderCustomNodeElement = useCallback(rd3tNodeProps => {
    ...
    const packageColor =
      PACKAGE_TIER_COLORS[packageTier] || PACKAGE_TIER_COLORS[0];
    ...
    return (
      <g transform={`translate(${xOffset}, ${yOffset})`}>
        ...
        {!isRoot &&
          !isEmpty &&
          (nodeDatum.attributes?.leftVolume ||
            nodeDatum.attributes?.rightVolume) && (
            <g>
              <text fill="#9CA3AF" textAnchor="middle" y={-32} fontSize={7}>
                L: ${(nodeDatum.attributes.leftVolume || 0).toLocaleString()}
              </text>
              <text fill="#9CA3AF" textAnchor="middle" y={-24} fontSize={7}>
        ...
        <circle
          ...
          style={{
            cursor: 'pointer',
            filter: isActive
              ? `drop-shadow(0 0 8px ${packageColor}40)`
              : 'none',
          }}
          onClick={toggleNode}
        />
   ...
   */
  const defaultNodeRenderer = useCallback(
    props => (
      <g>
  ...
  const treeConfigProps = useMemo(
    () => ({
      ...
      pathClassFunc: datum => {
        ...
        return position
          ? `binary-tree-link ${position}-leg`
          : 'binary-tree-link default-link';
      },
  ...
  return (
    <div
      className={`network-tree-visualization theme-${theme}`}
      ref={containerRef}
    >
      {/* Header with Stats */}
      {showStats && (
          ...
          <div className="tree-stats">
            ...
            <div className="stat-item">
              ...
              <span className="stat-value">
                ${(treeStats.totalVolume || 0).toLocaleString()}
              </span>
            </div>
            <div className="stat-item">
              <span className="stat-label">Left Leg</span>
              <span className="stat-value" style={{ color: '#FF6B35' }}>
                {treeStats.leftLegNodes} nodes | $
                {(treeStats.leftLegVolume || 0).toLocaleString()}
              </span>
            </div>
            <div className="stat-item">
              <span className="stat-label">Right Leg</span>
              <span className="stat-value" style={{ color: '#00D4FF' }}>
                {treeStats.rightLegNodes} nodes | $
                {(treeStats.rightLegVolume || 0).toLocaleString()}
              </span>
            </div>
          </div>
        </div>
      ...
      >
        <Tree ref={treeRef} {...treeConfigProps} />
      </div>
        ...
        <div className="node-details-panel">
          ...
          <div className="panel-content">
            ...
            <div className="detail-row">
              ...
              <span className="detail-value">
                {selectedNode.attributes?.downlineCount || 0}
              </span>
            </div>
            ...
            {Object.entries(PACKAGE_TIER_LABELS).map(([tier, label]) => (
              <div key={tier} className="legend-item">
                <div
                  className="legend-color"
                  style={{
                    backgroundColor: getPackageTierColor(parseInt(tier)),
                  }}
                ></div>
